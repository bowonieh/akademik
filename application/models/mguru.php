...
class Mguru extends CI_Model
{
  ...
  function pendidikan($id_guru){
   ...
   function _option_kelas() {
    ...
    function _option_guru() {
        $result = $this->db->get("guru");
        $kelas = array();
        foreach($result->result_array() as $row) {
        $kelas[$row["id_guru"]] = $row["nama_guru"];
        }
            return $kelas;
        }
...
function _option_tahun() {
   ...
   function _option_jenismapel() {
       //$this->db->order_by("id_tahun","ASC");
       $this->db->distinct('id_jenismapel');
   ...
   function hapus($id_guru){
       //$this->db->where(array('user.username'=>$id_guru,'username'=>$id_guru));
       //$this->db->join('user','user.username=guru.username','inner');
       //$this->db->delete(array('user','guru'));
       $this->db->query("DELETE FROM user where username = '$id_guru'");
       $this->db->query("DELETE FROM guru where username = '$id_guru'");
   }
  ...
  function getidguru($id){
      if($this->session->userdata('level')==='2'){
            ...
            //$this->db->join('user','user.username = guru.username','inner');
            $this->db->join('user','tatausaha.username = user.username','inner');
            $query  = $this->db->get_where('tatausaha',array('tatausaha.username' => $id));
            return $query;
      }
...
function addguru(){
    ...
    if(count($this->checkusername($username))>0){
...
function gantiusername($userlama,$userbaru){
    $this->db->update('user', array('username'=>$userbaru), array('username'=>$userlama),1);
    $this->db->update('guru', array('username'=>$userbaru), array('username'=>$userlama),1);
